...
final class UseArraySpreadRuleTest extends TestCase
{
    public function test_passing_fixtures_produce_no_violations(): void
    {
        self::assertSame([], $this->runFixtures(__DIR__.'/../Fixtures/use_array_spread/passing')->violations);
    }

    public function test_failing_fixtures_emit_violations(): void
    {
        $result = $this->runFixtures(__DIR__.'/../Fixtures/use_array_spread/failing');
        self::assertGreaterThanOrEqual(2, count($result->violations));
        foreach ($result->violations as $v) {
            self::assertSame('use-array-spread', $v->rule);
        }
    }
