...
use Sidetours\Compass\Rules\NumericSeparatorRule;

final class NumericSeparatorRuleTest extends TestCase
{
    public function test_passing_fixtures_produce_no_violations(): void
    {
        self::assertSame([], $this->runFixtures(__DIR__.'/../Fixtures/numeric_separator/passing')->violations);
    }

    public function test_failing_fixtures_emit_violations(): void
    {
        $result = $this->runFixtures(__DIR__.'/../Fixtures/numeric_separator/failing');
        self::assertGreaterThanOrEqual(4, count($result->violations));
        foreach ($result->violations as $v) {
            self::assertSame('numeric-separator', $v->rule);
        }
    ...
    private function runFixtures(string $dir): Result
    {
        $config = new Configuration(
            ...
            paths: [$dir],
            exclude: [],
            rules: [new NumericSeparatorRule()],
            ignore: [],
            baseline: null,
        );
