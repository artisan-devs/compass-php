...
use Sidetours\Compass\Rules\NeverReturnTypeRule;

final class NeverReturnTypeRuleTest extends TestCase
{
    public function test_passing_fixtures_produce_no_violations(): void
    {
        self::assertSame([], $this->runFixtures(__DIR__.'/../Fixtures/never_return_type/passing')->violations);
    }

    public function test_failing_fixtures_emit_violations(): void
    {
        $result = $this->runFixtures(__DIR__.'/../Fixtures/never_return_type/failing');
        self::assertGreaterThanOrEqual(2, count($result->violations));
        foreach ($result->violations as $v) {
            self::assertSame('never-return-type', $v->rule);
        }
    }
