---
name: fix-compass-numeric-literal-separator
description: Add underscore separators to large numeric literals. Use when Compass reports a `numeric-literal-separator` violation. Requires PHP 7.4+.
rule: numeric-literal-separator
...
# Fix `numeric-literal-separator` violations
...
Expect zero `numeric-literal-separator` rows for `<file>` and a green test suite. The transform is syntactic-only — tests should not regress.
