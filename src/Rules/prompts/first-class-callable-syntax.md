---
name: fix-compass-first-class-callable-syntax
description: Replace array-callable / string-callable arguments with first-class callable syntax. Use when Compass reports a `first-class-callable-syntax` violation. Requires PHP 8.1+.
rule: first-class-callable-syntax
...
# Fix `first-class-callable-syntax` violations
...
Expect zero `first-class-callable-syntax` rows for `<file>` and a green test suite.
