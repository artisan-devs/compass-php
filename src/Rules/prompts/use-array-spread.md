---
name: fix-compass-use-array-spread
description: Replace array_merge(...) calls with the array spread operator. Use when Compass reports a `use-array-spread` violation.
rule: use-array-spread
...
# Fix `use-array-spread` violations
...
Expect zero `use-array-spread` rows for `<file>` and a green test suite. If a test fails, the most likely cause is integer-key collision behaviour; revert and use `array_merge` with `// @compass-ignore-next-line use-array-spread`.
