---
name: fix-compass-match-expression
description: Convert switch statements with no fall-through into match expressions. Use when Compass reports a `match-expression` violation.
...
# Fix `match-expression` violations
...
Expect zero `match-expression` rows for `<file>` and green tests. If tests fail, suspect a loose-comparison change.
